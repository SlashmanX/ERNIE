...
sessions.push(new Session(1, "testSession", "true", "true", 1, "/main/"))
...
socket.sockets.on('connection', function(client){
	...
	client.on('createSession', function(data, callback) {
		var leader = myID;
		var session = new Session(sessions.length, data.name, data.public, data.bidirectional, leader, data.currentPage);
	...
	client.on('triggerClick', function(triggeredObject){
		...
		if(users[myID].getSession() != null)
		{
			...
			if(theSession.isBidirectional() || theSession.getLeader() == myID)
			{
